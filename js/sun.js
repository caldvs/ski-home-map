...
// ShadeMap API key — never committed to the repo. The deploy workflow
// writes `js/config.js` at build time from the SHADEMAP_API_KEY secret;
// that file is gitignored, so live users get the key without it ever
// appearing in git history. Local dev resolution chain:
//   1. ./config.local.js   (gitignored, your own key, dev-only import)
//   2. ./config.js          (generated by CI on deploy)
//   3. localStorage         (manual override: `localStorage.setItem(
//                            "ski:shademap-key","<jwt>")`)
...
async function _resolveShadeMapKey() {
  let key = "";
  // Dev-only: developer's personal config.local.js. Gated so this dynamic
  // import doesn't 404 on the deployed site.
  if (_isDevOrigin()) {
    try {
  ...
  // CI-generated config.js exists on deploy; absent locally unless the
  // developer chose to use this path instead of config.local.js.
  if (!key) {
    try {
      const mod = await import("./config.js");
      key = (mod.SHADEMAP_API_KEY || "").trim();
    } catch (e) { /* not generated yet — fine on initial dev install */ }
  }
  // Manual per-browser override via DevTools console.
